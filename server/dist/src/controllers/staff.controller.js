...
const academicPerformance_service_1 = require("../services/academicPerformance.service");
...
class StaffController {
    ...
    static async getDashboardSummary(req, res) {
        try {
            ...
            const activeStudents = await db_1.prisma.student.findMany({
                where: {
            ...
            const activeAssessments = await db_1.prisma.assessment.findMany({
                where: {
                    isActive: true,
                    AND: [
                        {
            ...
            const existingMarks = await db_1.prisma.mark.findMany({
                where: {
                    subjectId: { in: assignedSubjectIds },
                    student: {
                        sectionId: { in: assignedSectionIds },
            ...
            for (const a of assignments) {
                const applicableAssessments = activeAssessments.filter(ass => {
                    if (ass.academicYearId && ass.academicYearId !== a.academicYearId)
                        return false;
                    if (ass.departmentId && ass.departmentId !== a.section.departmentId)
                        return false;
                    if (ass.subjectId && ass.subjectId !== a.subjectId)
                        return false;
                    return true;
                });
            ...
            for (const a of assignments) {
                ...
                if (!subj.sections.some((s) => s.sectionId === a.sectionId)) {
                    subj.sections.push({
                        sectionId: a.sectionId,
                        sectionName: a.section.name,
            ...
            const recentMarksList = await db_1.prisma.mark.findMany({
     ...
     */
    static async getMarksForClass(req, res) {
        try {
            const staffId = req.user.id;
            ...
            if (req.user.role !== 'ADMIN') {
                const isAssigned = await (0, auth_middleware_1.checkStaffAssignment)(staffId, sectionId, subjectId, academicYearId);
                if (!isAssigned) {
    ...
    static async enterMarks(req, res) {
        try {
            ...
            if (req.user.role !== 'ADMIN') {
                const isAssigned = await (0, auth_middleware_1.checkStaffAssignment)(staffId, student.sectionId, subjectId, student.academicYearId);
                if (!isAssigned) {
                    return res.status(403).json({
            ...
            const existingMark = await db_1.prisma.mark.findUnique({
                where: {
            ...
            if (existingMark) {
                // Record change log
                await db_1.prisma.markChangeLog.create({
                    data: {
                ...
                mark = await db_1.prisma.mark.update({
                ...
                await db_1.prisma.auditLog.create({
                    data: {
                        userId: staffId,
                        action: audit_service_1.AuditAction.MARKS_UPDATED,
                        entity: 'Mark',
                        entityId: mark.id,
                        metadata: {
                            studentId,
                ...
                mark = await db_1.prisma.mark.create({
                    data: {
                        studentId,
                        subjectId,
                        assessmentId,
                ...
                await db_1.prisma.auditLog.create({
                    data: {
                        userId: staffId,
                        action: audit_service_1.AuditAction.MARKS_UPLOADED,
                        entity: 'Mark',
                        entityId: mark.id,
                        metadata: {
                            studentId,
     ...
     */
    static async getSectionPerformance(req, res) {
        try {
            const staffId = req.user.id;
            const sectionId = req.query.sectionId;
            const subjectId = req.query.subjectId;
            ...
            if (req.user.role !== 'ADMIN') {
                const isAssigned = await (0, auth_middleware_1.checkStaffAssignment)(staffId, sectionId, subjectId);
                if (!isAssigned) {
                    return res.status(403).json({
            ...
            const summary = await academicPerformance_service_1.AcademicPerformanceService.getSectionPerformanceFromDb(sectionId, subjectId);
            return res.status(200).json({
                sectionId,
                subjectId,
     ...
     */
    static async getStaffAssessments(req, res) {
        try {
            const staffId = req.user.id;
            ...
            if (sectionId || subjectId) {
                ...
                const [section, subject] = await Promise.all([
                    db_1.prisma.section.findUnique({ where: { id: sectionId } }),
                    db_1.prisma.subject.findUnique({ where: { id: subjectId } })
                ]);
                if (!section || !subject) {
            ...
            if (assignments.length === 0) {
                ...
                const globalAssessments = await db_1.prisma.assessment.findMany({
                    where: {
                        isActive: true,
                        academicYearId: null,
                        departmentId: null,
     ...
     */
    static async getAssignedSubjects(req, res) {
        try {
            const staffId = req.user.id;
            const assignments = await db_1.prisma.teacherAssignment.findMany({
                where: { staffId },
