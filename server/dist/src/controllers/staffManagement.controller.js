...
class StaffManagementController {
    ...
    static async getStaffList(req, res) {
        try {
            ...
            const whereClause = { role: client_1.Role.STAFF };
            if (status === 'active') {
            ...
            const formatted = staffList.map(s => ({
                ...
                defaultPasswordPreview: s.dateOfBirth ? (0, credential_util_1.generateDefaultStaffPassword)(s.name, s.dateOfBirth) : null,
                role: s.role,
                isActive: s.isActive,
    ...
    static async getStaffById(req, res) {
        try {
            const { id } = req.params;
            const staff = await db_1.prisma.user.findUnique({
                where: { id },
                select: {
                    id: true,
                    name: true,
            ...
            if (!staff || staff.role !== client_1.Role.STAFF) {
                return res.status(404).json({ error: 'Staff member not found', code: 'STAFF_NOT_FOUND' });
            }
            const defaultPassword = staff.dateOfBirth ? (0, credential_util_1.generateDefaultStaffPassword)(staff.name, staff.dateOfBirth) : null;
    ...
    static async createStaff(req, res) {
        try {
            ...
            if (rawDept) {
                targetDept = await db_1.prisma.department.findFirst({
                    where: {
                        OR: [
                            { id: String(rawDept) },
            ...
            if (effectivePassword) {
                const validation = (0, credential_util_1.validatePasswordComplexity)(effectivePassword);
                ...
                effectivePassword = (0, credential_util_1.generateDefaultStaffPassword)(name.trim(), trimmedDob);
            }
            ...
            let targetUsername = username ? String(username).trim().toLowerCase() : (0, credential_util_1.generateStaffUsername)(name.trim());
            if (!targetUsername) {
                targetUsername = trimmedEmail.split('@')[0];
            }
            const existingUserWithUsername = await db_1.prisma.user.findUnique({
            ...
            const passwordHash = await bcryptjs_1.default.hash(effectivePassword, salt);
            ...
            if (req.user) {
                await db_1.prisma.auditLog.create({
                    data: {
                        userId: req.user.id,
                        action: audit_service_1.AuditAction.STAFF_CREATED,
                        entity: 'User',
    ...
    static async updateStaff(req, res) {
        try {
            ...
            const staff = await db_1.prisma.user.findUnique({ where: { id } });
            if (!staff || staff.role !== client_1.Role.STAFF) {
                return res.status(404).json({ error: 'Staff member not found', code: 'STAFF_NOT_FOUND' });
            }
            const updateData = {};
            ...
            if (email && email.trim()) {
                ...
                if (trimmedEmail !== staff.email) {
                    const emailExists = await db_1.prisma.user.findUnique({ where: { email: trimmedEmail } });
                    if (emailExists) {
                        return res.status(409).json({
            ...
            if (username && username.trim()) {
                ...
                if (cleanUser !== staff.username) {
                    const uExists = await db_1.prisma.user.findUnique({ where: { username: cleanUser } });
                    if (uExists) {
            ...
            if (resetDefaultPassword) {
                ...
                const newPass = (0, credential_util_1.generateDefaultStaffPassword)(effectiveName, effectiveDob);
                const salt = await bcryptjs_1.default.genSalt(10);
                updateData.passwordHash = await bcryptjs_1.default.hash(newPass, salt);
     ...
     */
    static async resetStaffPassword(req, res) {
        try {
            const { id } = req.params;
     ...
     */
    static async authorizeStaffPassword(req, res) {
        try {
            const { id } = req.params;
            ...
            if (mode === 'REVOKE') {
                ...
                generatedPassword = (0, credential_util_1.generateDefaultStaffPassword)(staff.name, targetDob);
                const salt = await bcryptjs_1.default.genSalt(10);
                updateData.passwordHash = await bcryptjs_1.default.hash(generatedPassword, salt);
            ...
            return res.status(200).json({
                ...
                staff: updated,
                defaultPasswordPreview: generatedPassword || (updated.dateOfBirth ? (0, credential_util_1.generateDefaultStaffPassword)(updated.name, updated.dateOfBirth) : null)
            });
        }
        catch (error) {
     ...
     */
    static async getStaffCredentialsSummary(req, res) {
        try {
            const { id } = req.params;
            const staff = await db_1.prisma.user.findUnique({
                where: { id },
            ...
            const defaultPassword = staff.dateOfBirth
                ? (0, credential_util_1.generateDefaultStaffPassword)(staff.name, staff.dateOfBirth)
                : null;
            return res.status(200).json({
                staff: {
                    ...staff,
                    ...
                    loginMethods: {
                        byName: staff.name,
                        byUsername: staff.username || (0, credential_util_1.generateStaffUsername)(staff.name),
                        byEmail: staff.email
                    }
                }
            });
     ...
     */
    static async discontinueStaff(req, res) {
        try {
            const { id } = req.params;
     ...
     */
    static async reissueStaffCredentials(req, res) {
        try {
            const { id } = req.params;
            ...
            if (!effectivePassword) {
                ...
                effectivePassword = (0, credential_util_1.generateDefaultStaffPassword)(staff.name, effectiveDob);
            }
            else {
                const validation = (0, credential_util_1.validatePasswordComplexity)(effectivePassword);
                if (!validation.isValid) {
    ...
    static async assignStaff(req, res) {
        try {
            const staffId = req.params.id || req.body?.staffId || req.body?.staff_id;
            ...
            if (!sectionId && req.body?.class_section_id)
                sectionId = req.body.class_section_id;
            ...
            // 1. Verify Staff
            const staff = await db_1.prisma.user.findUnique({ where: { id: staffId } });
            if (!staff || staff.role !== client_1.Role.STAFF) {
                return res.status(404).json({ error: 'Staff member not found', code: 'STAFF_NOT_FOUND' });
            }
            // 2. Verify Subject
            ...
            if (!academicYearId) {
                const currentYear = await db_1.prisma.academicYear.findFirst({
            ...
            let section = await db_1.prisma.section.findUnique({
                where: { id: sectionId },
                include: { department: true, year: true }
            });
            if (!section) {
            ...
            if (section.departmentId !== subject.departmentId || section.yearId !== subject.yearId) {
                let matchingSection = await db_1.prisma.section.findFirst({
                    where: {
                        departmentId: subject.departmentId,
                        yearId: subject.yearId,
                        academicYearId,
                ...
                if (!matchingSection) {
                    matchingSection = await db_1.prisma.section.create({
                        data: {
            ...
            const existingAssignment = await db_1.prisma.teacherAssignment.findUnique({
                where: {
            ...
            if (existingAssignment) {
                ...
                const reassigned = await db_1.prisma.teacherAssignment.update({
                ...
                if (req.user) {
                    await db_1.prisma.auditLog.create({
                        data: {
                            userId: req.user.id,
                            action: audit_service_1.AuditAction.STAFF_ASSIGNMENT_CHANGED,
            ...
            const newAssignment = await db_1.prisma.teacherAssignment.create({
                data: {
                    staffId,
                    subjectId,
                    sectionId,
    ...
    static async updateStaffAssignment(req, res) {
        try {
            ...
            if (!sectionId && req.body?.class_section_id)
                sectionId = req.body.class_section_id;
            const assignment = await db_1.prisma.teacherAssignment.findUnique({
            ...
            const newSubject = await db_1.prisma.subject.findUnique({
            ...
            let targetSection = await db_1.prisma.section.findUnique({
            ...
            if (targetSection && targetSection.yearId !== newSubject.yearId) {
                const matchingSection = await db_1.prisma.section.findFirst({
                    where: {
                ...
                }) || await db_1.prisma.section.findFirst({
                    where: {
                        departmentId: newSubject.departmentId,
                        yearId: newSubject.yearId,
                        academicYearId: targetAcademicYearId,
                ...
                if (matchingSection) {
                    ...
                    targetSection = await db_1.prisma.section.findUnique({
                        where: { id: targetSectionId },
                        include: { department: true, year: true }
                    });
                }
            ...
            if (targetSubjectId !== assignment.subjectId || targetSectionId !== assignment.sectionId || targetAcademicYearId !== assignment.academicYearId) {
                const conflict = await db_1.prisma.teacherAssignment.findUnique({
                    where: {
                        subjectId_sectionId_academicYearId: {
            ...
            const updated = await db_1.prisma.teacherAssignment.update({
                where: { id: assignmentId },
                data: {
                    subjectId: targetSubjectId,
                    sectionId: targetSectionId,
    ...
    static async deleteStaffAssignment(req, res) {
        try {
            ...
            await db_1.prisma.teacherAssignment.delete({ where: { id: assignmentId } });
            if (req.user) {
                await db_1.prisma.auditLog.create({
                    data: {
                        userId: req.user.id,
    ...
    static async deleteStaff(req, res) {
        try {
            ...
            const [marksCount, changeLogsCount] = await Promise.all([
                db_1.prisma.mark.count({ where: { enteredBy: id } }),
                db_1.prisma.markChangeLog.count({ where: { changedBy: id } })
            ]);
            ...
            await db_1.prisma.teacherAssignment.deleteMany({ where: { staffId: id } });
            // Preserve audit logs by reassigning userId to the administering user
            if (req.user) {
                await db_1.prisma.auditLog.updateMany({
                    where: { userId: id },
